        ...
        <table class="cart-list">
            ...
            <c:forEach items="${cartItems}" var="item">
                <tr select="false" stock="${item.product.stock}" buy="${item.number}"
                        price-per="${item.product.NPrice}" ciid="${item.id}">
                    <input type="hidden" name="ciids" value="${item.id}">
                    <td>
                        <img src="img/noselect.png" class="select-img">
                        <img src="${productImgDir}${item.product.coverList[0].urlImage}" class="cart-item-jpg">
                    </td>
                    <td class="item-name">
                        <a class="cart-item-title" href="product?product.id=${item.product.id}">${item.product.name}</a>
                    ...
                    <td>
                        <span class="cart-item-old-price">￥${item.product.OPrice}</span>
                        <span class="cart-item-now-price">￥${item.product.NPrice}</span>
                    </td>
                    <td>
                        <div class="cart-change">
                            <a href="javascript:void(0);" class="cart-decrease cart-change-button">-</a><input
                                value="${item.number}" class="item-num-input"><a href="javascript:void(0);" class="cart-increase cart-change-button">+</a>
                        </div>
                    </td>
                    <td>
                        <span class="cart-item-sum">￥${item.product.NPrice}</span>
                    </td>
                    <td>
                        <a href="del?cart.id=${item.id}" class="delete-button">删除</a>
                    </td>
                </tr>
            </c:forEach>
