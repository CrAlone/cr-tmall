...
<script>
    $(function () {
        ...
        $(".address-table input,.address-table textarea").keyup(function () {
            $("#address-text").text($("textarea[name='order.address']").val());
            $("#name-and-phone").text($("input[name='order.receiver']").val()+" "+$("input[name='order.mobile']").val());
...
<main class="buy-page">
    <form action="createOrder" method="post">
        ...
        <table class="address-table">
            ...
            <tr>
                ...
                <td class="right-column"><textarea placeholder="建议您如实填写详细收货地址，例如接到名称，门牌号码，楼层和房间号等信息"
                                                   name="orderTable.address"></textarea></td>
            </tr>
            <tr>
                <td class="left-column">邮政编码</td>
                <td class="right-column"><input type="text" placeholder="如果您不清楚邮递区号，请不要填写" name="orderTable.postcode"></td>
            </tr>
            <tr>
                <td class="left-column">收货人姓名<span class="red-star">*</span></td>
                <td class="right-column"><input type="text" placeholder="长度不超过25个字符" name="orderTable.receivers"></td>
            </tr>
            <tr>
                <td class="left-column">手机号码 <span class="red-star">*</span></td>
                <td class="right-column"><input type="text" placeholder="请输入11位手机号码" name="orderTable.mobile"></td>
            </tr>
            </tbody>
        </table>
        ...
        <table class="buy-list">
            ...
            <tr class="buy-item">
                <td class="buy-item-image"><img src="${productImgDir}${item.product.coverList[0].urlImage}"></td>
                ...
                <td class="buy-item-price">
                    <span>￥${item.product.NPrice}</span>
                </td>
                <td class="buy-item-num">
                    <span>${item.number}</span>
                </td>
                <td class="buy-item-sum">
                    <span>￥${item.product.NPrice * item.number}</span>
                </td>
        ...
        <table class="sum-bar">
            <tr>
                <td rowspan="2" width="45%" class="comment-td">给卖家留言：
                    <textarea class="comment-input" name="orderTable.message" placeholder="选填:对本次交易的说明(建议填写已和卖家协商一致的内容)"></textarea>
                </td>
                <td>
                    运送方式：
